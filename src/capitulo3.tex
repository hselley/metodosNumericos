...
$\forall x\in C$. Sea $p_0\in\mathbb{R}$ y $p_0\in C$ entonces:  
	\[p_{n} = p_{n-1} - \frac{f(p_{n-1})}{f'(p_{n-1})},\quad \forall n\geq 1\]
	\label{eq:Newton}
\end{theorem}
...
\subsection{Análisis geométrico/algebraico}
Existe otra forma de explicar como se encuentra la expresión general de Newton-Rapshon \ref{eq:Newton} de forma algebraica con
conceptos geométricos simples.

Para ello, se utiliza como punto de partida de un ejemplo particular para posteriormente llegar a un caso general.

\begin{exerciseT}
	Considere la función $f(x)=x^2-4$. Se desea encontrar una aproximación a una raíz de $f$, que de antemano se sabe
	que tiene dos raíces en $x=\pm 2$. La gráfica de la función se muestra en la Figura \ref{fig:NewtonRaphson01}.
	...
	\begin{figure}[H]
		...
		\begin{tikzpicture}
			...
			\draw[blue,smooth,domain=-2.5:2.5] plot (\x,{\x*\x-4});
			\node at (2,2.5) {$y = x^2-4$};
		\end{tikzpicture}
		\caption{Gráfica de la función $f(x)=x^2-4$.}
		\label{fig:NewtonRaphson01}
	\end{figure}

	Para encontrar la primer aproximación del método de Newton-Raphson a la raíz $x=2$, se elige un punto
	inicial con el cual se trazará una recta. Para obtener la ecuación de la recta se necesita de un punto y
	la pendiente, para ello se necesita de la ecuación punto-pendiente (\ref{eq:punto-pendiente}).
	\begin{equation}
		y-y_0 = m(x-x_0)
		\label{eq:punto-pendiente}
	\end{equation}

	El punto se elegirá de manera arbitraria, considerando que mientras más cerca se encuentre de la raíz más
	rápidamente el método se aproximará a ella. El punto inicial elegido será $x_0=1$ y la coordenada completa 
	al sustituir $x_0$ en la función $f(x)$ es \[P_0 = (1, -3).\]

	Para la pendiente de la recta usaremos la derivada, recordando que la derivada evaluada en un punto geometricamente 
	corresponde a la pendiente de la recta tangente a ese punto. La derivada de la función $f(x)=x^2-4$ es 
	\begin{equation}
		f'(x) = 2x
		\label{ex:NewtonRaphson02}
	\end{equation}

	Ahora, se obtiene la pendiente y la ecuación de la recta para el punto inicial $P_0 = (1, -3)$.
	\begin{align*}
		& m = f'(1) = 2(1) = 2 \\
		& y-(-3) = 2(x-1) \\
		& y+3=2x-2\\
	...
	Continuando con el método, ahora se obtiene la raíz de la ecuación de la recta. Esto es:
	\begin{align*}
		& y = 2x - 5 = 0 \\
		& x = \dfrac{5}{2} = 2.5
	\end{align*}
	Este valor de $x=\frac{5}{2}$ corresponde a la primer aproximación del método a la raíz de la función. 
	La gráfica la recta junto con la función $f(x)$ en un mismo plano se muestra en la Figura \ref{fig:NewtonRaphson02} 
	donde se puede observar lo que el método ha realizado para obtener esta aproximación.
	...
	\begin{figure}[H]
		...
		\begin{tikzpicture}
			...
			\draw[blue,smooth,domain=-2.5:2.5] plot (\x,{\x*\x-4});
			\draw[red,smooth,domain=0:3.5] plot (\x,{2*\x-5});
			%\node at (2,2.5) {$y = x^2-4$};

			% Punto resaltado en (1,-3)
			\fill[gray] (1,-3) circle [radius=2pt];
			\node[right, black, font=\small] at (1,-3) {$P_0=(1,-3)$};

			% Punto resaltado en (5/2,0)
			\fill[gray] (5/2,0) circle [radius=2pt];  % círculo sólido rojo
			\node[above right, black, font=\small] at (5/2,0) {$x=\frac{5}{2}$};

		\end{tikzpicture}
		\caption{Primer aproximación del método a la raíz de $f$.}
		\label{fig:NewtonRaphson02}
	\end{figure}

	En la Figura \ref{fig:NewtonRaphson02} se puede observar en rojo la recta que se obtuvo. La recta en efecto
	es tangente a la función $f(x)=x^2-4$ en el punto $P_0 = (1, -3)$ y tiene raíz en $x=\frac{5}{2}$.

	A continuación se repite el procedimiento empleando el punto $x=\frac{5}{2}$ y $P_1=(\frac{5}{2}, \frac{9}{4})$ 
	luego de evaluarlo en $f$.

	\begin{align*}
		& m = f'\left(\frac{5}{2}\right) = 2\left(\frac{5}{2}\right) = 5 \\
		& y-\left(\frac{9}{4}\right) = 5\left(x-\frac{5}{2}\right) \\
		& y = 5x - \dfrac{41}{4} = 0 \\
		& x = \dfrac{41}{20} = 2.05
	\end{align*}

	El valor $x = \frac{41}{20}$ corresponde a la segunda aproximación del método a la raíz. Se grafica de nuevo la recta 
	y la función $f$ y se muestra en la Figura \ref{fig:NewtonRaphson03}.
		...
		\begin{tikzpicture}
			...
			\draw[blue,smooth,domain=-2.8:2.8] plot (\x,{\x*\x-4});
			\draw[red,smooth,domain=1.2:2.8] plot (\x,{5*\x-41/4});
			%\node at (2,2.5) {$y = x^2-4$};

			% Punto resaltado en (5/2,9/4)
			\fill[gray] (5/2,9/4) circle [radius=2pt];
			\node[right, black, font=\small] at (5/2,9/4) {$P_1=(\frac{5}{2}, \frac{9}{4})$};

			% Punto resaltado en (41/20,0)
			\fill[gray] (41/20,0) circle [radius=2pt];  % círculo sólido rojo
			\node[above right, black, font=\small] at (41/20,0) {$x=\frac{41}{20}$};

		\end{tikzpicture}
		\caption{Segunda aproximación del método a la raíz de $f$.}
		\label{fig:NewtonRaphson03}
	\end{figure}

	Como se puede observar en la Figura \ref{fig:NewtonRaphson03}, la aproximación $x = \dfrac{41}{20}$ ya se encuentra
	muy cerca de la raíz de la función. Se debe repetir el procedimiento hasta que se cumpla con la tolerancia establecida.
	\label{ex:NewtonRaphson04}
\end{exerciseT}


\subsubsection{Generalización}
Ahora se realizará una generalización del procedimiento descrito en el Ejercicio \ref{ex:NewtonRaphson04}.
El punto de partida es la ecuación punto-pendiente (\ref{eq:punto-pendiente}), se sustituye en la pendiente la derivada de la 
función y luego se hace un poco de álgebra.

\begin{align*}
	& y - y_0 = m(x-x_0) \\
	& y - f(x_0) = f'(x_0)(x-x_0) \\
	\intertext{Se iguala a cero para obtener la raíz de la recta}
	& y = f(x_0) + f'(x_0)(x-x_0) = 0\\ 
	& f'(x_0)(x-x_0) = -f(x_0) \\
	\intertext{Se despeja $x$}
	& x = x_0 - \dfrac{f(x_0)}{f'(x_0)} = x_1
	\intertext{Donde $x_1$ es la primer aproximación a la raíz}
\end{align*}

Escribiendo esta última expresión en términos de la $n$-ésima aproximación.

\begin{equation}
x_n = x_{n-1} - \frac{f(x_{n-1})}{f'(x_{n-1})}, \quad \forall \quad n\geq 1.
\end{equation}
...
Al aplicar esta aproximación para $f'(p_{n-1})$ en la fórmula de Newton, se obtiene y formaliza en el teorema \ref{teo:secante}.\\
...
\begin{figure}[H]
	\centering
    \includegraphics[scale=0.2]{../img/metodoSecante.jpg}
...
\subsection{Análsis geométrico/algebraico}

Otra alternativa de explicar como funciona el método de la secante es mediante un análisis geométrico empleando álgebra simple
en el procedimiento.

Al igual que se hizo con el método de Newton-Raphson, se utiliza un ejemplo particular para llegar a un caso general.


\begin{exerciseT}

	Considere la función $f(x)=x^2-4$. Se desea encontrar una aproximación a una raíz de $f$, que de antemano se sabe
	que tiene dos raíces en $x=\pm 2$. La gráfica de la función se muestra en la Figura \ref{fig:Secante01}.
	...
	Para encontrar la primer aproximación del método de la Secante a la raíz $x=2$, se deben eligir un par de puntos
	iniciales $P_0 = (x_0, y_0)$ y $P_1 = (x_1, y_1)$ con los cuales se trazará una recta secante a $f(x)$. 
	
	Con este par de puntos se puede obtener la pendiente de la recta secante a $f(x)$ empleando la expresión (\ref{eq:pendienteRecta}):
	\begin{equation}
		m = \dfrac{y_2-y_1}{x_2-x_1}	
		\label{eq:pendienteRecta}
	\end{equation}

	Ahora para obtener la ecuación de la recta se utilizará el punto $P_1$ y la pendiente $m$, haciendo uso de la ecuación punto-pendiente 
	(\ref{eq:punto-pendiente}).
	

	Los puntos iniciales $P_0 = (x_0, y_0)$ y $P_1 = (x_1, y_1)$ se deben elegir de forma arbitraria, considerando que mientras más cerca se 
	encuentren de la raíz más rápidamente el método se aproximará a ella. Para este ejemplo se eligen $x_0=1$ y $x_1=3$, cuyas coordenadas 
	completas son $P_0 = (1, -3)$ y $P_1 = (3, 5)$.

	Ahora, se obtiene la pendiente y la ecuación de la recta para los puntos iniciales $P_0 = (1, -3)$ y $P_1 = (3, 5)$.
	\begin{align*}
		& m = \dfrac{5-(-3)}{3-1} = 4\\
		\intertext{Se obtiene la ecuación de la recta con la pendiente $m$ y el punto $P_1$}:
		& y-(5) = 4(x-3) \\
		& y-5=4x-12\\
		& y = 4x - 7
	\end{align*}
	...
	Este valor de $x=\frac{7}{4}$ corresponde a la primer aproximación del método a la raíz de la función. 
	La gráfica la recta junto con la función $f(x)$ en un mismo plano se muestra en la Figura \ref{fig:Secante02} 
	donde se puede observar lo que el método ha realizado para obtener esta aproximación.
	...
	\begin{figure}[H]
		...
		\begin{tikzpicture}
			...
			\draw[blue,smooth,domain=-2.5:3.5] plot (\x,{\x*\x-4});
			\draw[red,smooth,domain=0.5:3.8] plot (\x,{4*\x-7});
			%\node at (2,2.5) {$y = x^2-4$};
			...
			% Punto resaltado en (3,5)
			\fill[gray] (3,5) circle [radius=2pt];
			\node[right, black, font=\small] at (3,5) {$P_1=(3,5)$};

			% Punto resaltado en (7/4,0)
			\fill[gray] (7/4,0) circle [radius=2pt];  % círculo sólido rojo
			\node[above left, black, font=\small] at (7/4,0) {$x=\frac{7}{4}$};
	...
	En la Figura \ref{fig:Secante02} se puede observar en rojo la recta que se obtuvo. La recta en efecto
	es secante a la función $f(x)=x^2-4$ en el punto $P_0 = (1, -3)$ y $P_1 = (3, 5)$, y además tiene raíz 
	en $x=\frac{7}{4}$.

	A continuación se repite el procedimiento empleando $x=\frac{7}{4}$ como nueva aproximación y se tienen 
	los puntos iniciales $P_1=\left(3,5\right)$ y $P_2=\left(\frac{7}{4}, -\frac{15}{16}\right)$.

	\begin{align*}
		& m = \dfrac{-\frac{15}{16} - 5}{\frac{7}{4} - 3} = \dfrac{19}{4} \\
		& y-\left(-\frac{15}{16}\right) = \dfrac{19}{4}\left(x-\dfrac{7}{4}\right) \\
		& y = \dfrac{19}{4}x - \dfrac{37}{4} = 0 \\
		& x = \dfrac{\frac{37}{4}}{\frac{19}{4}} = \dfrac{37}{19} = 1.9473\dots
	\end{align*}

	El valor $x = \frac{37}{19}$ corresponde a la segunda aproximación del método a la raíz. Las gráficas de la  
	recta y la función $f$ se muestran en la Figura \ref{fig:Secante03}.
	...
	\begin{figure}[H]
		...
		\begin{tikzpicture}
			...
			\draw[blue,smooth,domain=-2.5:3.3] plot (\x,{\x*\x-4});
			\draw[red,smooth,domain=1:3.3] plot (\x,{19/4*\x-37/4});
			%\node at (2,2.5) {$y = x^2-4$};
			...
			% Punto resaltado en (7/4, -15/16)
			\fill[gray] (7/4,-15/16) circle [radius=2pt];
			\node[right, black, font=\small] at (7/4,-15/16) {$P_2=(\frac{7}{4},-\frac{15}{16})$};

			% Punto resaltado en (37/19,0)
			\fill[gray] (37/19,0) circle [radius=2pt];  % círculo sólido rojo
			\node[above left, black, font=\small] at (37/19,0) {$x=\frac{37}{19}$};
	...
	Como se puede observar en la Figura \ref{fig:Secante03}, la aproximación $x = \frac{37}{19}$ ya se encuentra
	muy cerca de la raíz de la función. Se debe repetir el procedimiento hasta que se cumpla con la tolerancia establecida.
	\label{ex:Secante04}
...
\subsubsection{Generalización}
Ahora se realizará una generalización del procedimiento descrito en el Ejercicio \ref{ex:Secante04}.
El punto de partida con los puntos $P_0 = \left(x_0, y_0\right)$ y $P_1 = \left(x_1, y_1\right)$, se hace el 
cálculo de la pendiente con la expresión (\ref{eq:pendienteRecta}) y la obtención de la ecuación 
punto-pendiente (\ref{eq:punto-pendiente}), se sustituye y luego se hace un poco de álgebra.

\begin{align*}
	& m = \dfrac{y_1 - y_0}{x_1-x_0}\\
	& y - y_1 = \dfrac{y_1 - y_0}{x_1-x_0}(x-x_1) \\
	& y = f(x_1) + \dfrac{f(x_1) - f(x_0)}{x_1-x_0}(x-x_1) \\
	\intertext{Se iguala a cero para obtener la raíz de la recta}
	& y = f(x_1) + \dfrac{f(x_1) - f(x_0)}{x_1-x_0}(x-x_1) = 0\\ 
	\intertext{Se despeja $x$}
	& \dfrac{f(x_1) - f(x_0)}{x_1-x_0}(x-x_1) = -f(x_1)\\
	& x = x_1 - f(x_1)\dfrac{x_1-x_0}{f(x_1) - f(x_0)} = x_2
	\intertext{Donde $x_2$ es la primer aproximación del método a la raíz}
\end{align*}

Escribiendo esta última expresión en términos de la $n$-ésima aproximación.

\begin{equation}
x_n = x_{n-1} - f(x_{n-1})\dfrac{x_{n-1}-x_{n-2}}{f(x_{n-1}) - f(x_{n-2})}, \quad \forall \quad n\geq 2.
\end{equation}
